...
<html>
  <head>
    <meta charset="utf-8">
    <title>Kasir <?php echo $app->get_app('company_name'); ?></title>
  ...
  <body>
        ...
        <nav id="menu-atas" style="text-align:center;">
          <ul id="nav-atas">
          	<li><a href="index_kasir.php?p=laman_utama_kasir">Halaman Utama</a></li>
          	<li><a href="?p=laman_transaksi_kasir">Transaksi</a></li>
          	<li><a href="login.php?keluar=true">Keluar</a></li>
          </ul>
        </nav>
  			<div class="clear"></div>
  			<hr>
          ...
          if ( ! empty($_GET['p']) ) :
              ...
              require_once('views/laman_utama_kasir.php');
          endif;
          ?>
  			</section>
  			<hr>
  ...
  <script>
    ...
    function delete_transaksi(ID) {
        ...
        if (r == true) {
            window.location = "?p=laman_transaksi_kasir&delete=" + ID;
        } else {
            console.log("Batal menghapus");
        }
    }
    function ubah_transaksi(ID, qty) {
        ...
        if (data != null) {
            window.location = "?id="+ ID +"&p=laman_transaksi_kasir&update=true&qty=" + data;
        }
        return false;
    }
