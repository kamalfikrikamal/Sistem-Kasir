...
<div class="grid_9" style="margin-left: 25%;">
	<form action="index_kasir.php" method="get">
		...
		<input type="hidden" name="p" value="laman_transaksi_kasir">
...
<table width="100%">
	...
	?>
		<tr>
			...
			<td> 
			<a href="#" onclick="ubah_transaksi('<?php echo $row['id']; ?>', '<?php echo $row['qty'] ?>');"  class="text-blue">Edit Jumlah</a> | <a href="#" onclick="delete_transaksi('<?php echo $row['id']; ?>');" class="text-red">Hapus</a></td>
		</tr>
	...
	<tfoot>
		<tr>
			...
			<td style=" text-align: right;"><?php echo (empty($_SESSION['produk'])) ? '' : number_format($total) ?></td>
			<td></td>
		</tr>
	</tfoot>
</table>
<div style="width: 50%; float: left; margin-top:15px;">
	<a href="?p=laman_transaksi_kasir&destroy=true" class="tombol">Batalkan Transaksi</a>
</div>
...
<script>
        var newwindow = window.open('print_nota_kasir.php?id=' + <?php echo $_GET['cetak'] ?>,'name','height=600,width=800');
        if (window.focus) {
            newwindow.focus()
        }
</script>
